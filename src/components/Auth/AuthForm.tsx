'use client';

import { useFormStatus } from 'react-dom';
import { useTransition } from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';

interface AuthFormProps {
  mode: 'login' | 'register';
  /** Server action called with email+password FormData */
  onEmailSubmit: (formData: FormData) => Promise<{ error?: string; success?: string } | void>;
  /** Called when user clicks "Continue with Google" */
  onGoogleSignIn: () => Promise<{ error?: string } | void>;
  message?: string;
  isSuccess?: boolean;
}

function SubmitButton({ label, extraDisabled }: { label: string; extraDisabled?: boolean }) {
  const { pending } = useFormStatus();
  const isDisabled = pending || extraDisabled;
  return (
    <Button type="submit" className="w-full" disabled={isDisabled}>
      {isDisabled ? 'Please wait…' : label}
    </Button>
  );
}

export function AuthForm({
  mode,
  onEmailSubmit,
  onGoogleSignIn,
  message,
  isSuccess,
}: AuthFormProps) {
  const [isGooglePending, startGoogleTransition] = useTransition();
  const [isEmailPending, startEmailTransition] = useTransition();

  function handleGoogleClick() {
    startGoogleTransition(async () => {
      await onGoogleSignIn();
    });
  }

  function handleEmailSubmit(formData: FormData) {
    startEmailTransition(async () => {
      await onEmailSubmit(formData);
    });
  }

  return (
    <div className="space-y-4">
      {/* Google OAuth – plain button, not a form */}
      <Button
        variant="outline"
        className="w-full"
        type="button"
        disabled={isGooglePending}
        onClick={handleGoogleClick}
      >
        <svg className="mr-2 size-4" viewBox="0 0 24 24" aria-hidden="true">
          <path
            fill="currentColor"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="currentColor"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="currentColor"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="currentColor"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        {isGooglePending ? 'Redirecting…' : 'Continue with Google'}
      </Button>

      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <span className="w-full border-t" />
        </div>
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-card px-2 text-muted-foreground">or</span>
        </div>
      </div>

      {/* Email + password */}
      <form action={handleEmailSubmit} className="space-y-3">
        <div className="space-y-1">
          <Label htmlFor="email">Email</Label>
          <Input
            id="email"
            name="email"
            type="email"
            placeholder="you@example.com"
            required
            autoComplete="email"
          />
        </div>
        <div className="space-y-1">
          <Label htmlFor="password">Password</Label>
          <Input
            id="password"
            name="password"
            type="password"
            placeholder="••••••••"
            required
            autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
            minLength={6}
          />
        </div>
        {message && (
          <p className={`text-sm ${isSuccess ? 'text-green-600' : 'text-destructive'}`}>
            {message}
          </p>
        )}
        <SubmitButton
          label={mode === 'login' ? 'Sign in' : 'Create account'}
          extraDisabled={isEmailPending}
        />
      </form>
    </div>
  );
}
